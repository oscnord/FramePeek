# FramePeek – GitHub Copilot Instructions

This document provides context for GitHub Copilot to understand, navigate, and extend the FramePeek codebase efficiently.
...
- **VideoInfoLoader.swift** – Orchestrates metadata extraction from AVAsset via `getExtendedInfo(url:asset:)`.
- **AudioInfoLoader.swift** – `loadAudioInfo(asset:)` → returns `[AudioTrackInfo]` with codec, channels, sample rate, bitrate, language.
- **GenerateKeyframeThumbnails.swift** – Thumbnail generation utilities.

#### Utils/Analysis/ – Frame and bitrate analysis

- **ExtractFramesStream.swift** – AsyncStream-based frame extraction utilities.
...
- **FastBitrateExtractor.swift** – Main bitrate extraction orchestrator with format-specific optimizations.
...
- **TSBitrateExtractor.swift** – Specialized extraction for Transport Stream files.
...
- **KeyframeMarker.swift** – Keyframe detection using sync sample flags.
- **VUIParser.swift** – Video Usability Information parsing.

### Models

- **Models/MediaModels.swift** – Core data models:
  - **ExtendedVideoInfo** – All video metadata fields.
  - **AudioTrackInfo** – Per-track audio info.
  - **FrameAnalysisResult** – Batch result with samples and FPS stats.
  - **FrameSamplingOptions** – Configures sampling behavior.
...
The following fields are populated by `getExtendedInfo`:

- **File and container**: `fileName`, `fileSize`, `fileSizeBytes`, `overallBitrate`, `duration`, `durationFormatted`, `containerFormat`, `containerFormatProfile`, `codecIdRaw`
- **Video geometry and timing**: `resolution`, `displayAspectRatio`, `frameRate`, `orientationDegrees`, `pixelAspectRatio`, `cleanAperture`, `scanType`, `frameRateMode`
- **Codec and stream details**: `codec`, `codecProfile`, `codecIdInfo`, `trackBitrate`, `maxBitrate`, `bitsPerPixelFrame`, `videoStreamSize`
- **Color and HDR**: `colorSpace`, `chromaSubsampling`, `bitDepth`, `colorPrimaries`, `transferFunction`, `matrixCoefficients`, `colorRange`, `hdrFormat`
- **AV1 specifics**: `av1CSize`, `av1Profile`, `av1Level`, `av1ChromaSubsampling`, `av1FullRange`
- **Metadata**: `creationDate`, `metadataTitle`, `metadataArtist`, `metadataEncoder`, `metadataDescription`
- **Audio**: `audioTracks: [AudioTrackInfo]`

## Control flow of getExtendedInfo(url:asset:)

1. **File-level info** – Derives file name, size, and overall bitrate.
2. **Duration and basic metadata** – Loads duration and common metadata.
3. **Video track discovery** – Loads first video track and reads properties.
4. **Format description parsing** – Extracts codec, color info, and sample description atoms.
5. **Derived metrics** – Computes DAR, HDR format, bits-per-pixel, etc.
6. **Audio tracks** – Delegates to `loadAudioInfo(asset:)`.
7. **Aggregation** – Returns `ExtendedVideoInfo` with all available fields.

## Frame Analysis Flow

The app supports three sampling modes:
...
- Built for **macOS 15.2+** (Sequoia) using SwiftUI and AppKit.
...
- Minimum deployment target: macOS 15.2.
...
- **Always use localization** for user-facing strings:
  - `Text()` views with string literals automatically localize.
  - For computed strings and `.help()` modifiers, use `String(localized:)`.
  - See `Localizable.xcstrings` for localization entries.
...
5. **Enum Display Names**: For enum `displayName` properties, use `String(localized:)`.
